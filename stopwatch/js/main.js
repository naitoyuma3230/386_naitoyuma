...
　//現在時刻からスタート時の時刻を引き経過時間を計算する。elapsedTimeを足し計測再開時に対応する。
  ...
  setBttonStateRunning(); //ストップボタンのみアクティブにする。他はインアクティブ
    startTime = Date.now();　//スタート時刻をstartTimeに代入してから
    countUp();　//カウントアップを開始する
    isRunning = true;　//カウントアップ起動中に設定
  })

  //ストップボタンを押した時の動き
  stop.addEventListener('click',()=>{
    setBttonStateStopped(); //スタート、リセットボタンをアクティブ、ストップボタンをインアクティブ
